...
namespace EmployeeManagementBackend.Controllers
{
    ...
    public class AttendanceController : ControllerBase
    {
        ...
        public AttendanceController(IAttendanceRepository attendanceRepo, UserManager<ApplicationUser> userManager,
        ...
        public async Task<IActionResult> CheckIn()
        {
            ...
            if (cairoNow.TimeOfDay < checkInStart || cairoNow.TimeOfDay > checkInEnd)
            {
                ...
                return BadRequest(new { message = $"Check-in is only allowed between {startStr} and {endStr} (Cairo time)." });
            }
            ...
            if (attendance == null)
            {
                attendance = new Attendance { EmployeeId = user.Id, Date = today, IsPresent = true };
            ...
            var jobDelay = TimeSpan.FromSeconds(30  );
              ...
              _backgroundJobClient.Schedule<IAttendanceJobService>(svc => svc.AutoCheckoutAsync(attendance.Id),jobDelay);
        ...
        public async Task<IActionResult> GetHistory(int pageIndex = 0, int pageSize = 10)
        {
            ...
            var (data, total) = await _attendanceRepo.GetEmployeeHistoryAsync(user.Id, pageIndex, pageSize);
            return Ok(new
            {
                Data = data,
                PageIndex = pageIndex,
        ...
        public async Task<IActionResult> GetWeeklySummary(DateTime? startDate = null)
        {
            ...
            var attendances = (await _attendanceRepo.GetAttendancesInRangeAsync(startDateOnly, endDateOnly))
            ...
            return Ok(new
            {
                ...
                {
                    Date = a.Date,
                    CheckInTime = a.CheckInTime.HasValue ? TimeHelper.ConvertUtcToCairo(a.CheckInTime.Value) : (DateTime?)null,
                    CheckOutTime = a.CheckOutTime.HasValue ? TimeHelper.ConvertUtcToCairo(a.CheckOutTime.Value) : (DateTime?)null,
                    Hours = a.TotalHoursWorked != null ? Math.Round(a.TotalHoursWorked.Value.TotalHours, 2) : 0
                })
            });
        }
        ...
        public async Task<IActionResult> GetDailyAttendance(DateTime? date = null, int pageIndex = 0, int pageSize = 10)
        {
            if (date == null) date = TimeHelper.GetCairoNow();
            var targetDate = DateOnly.FromDateTime(date.Value);

            var (data, total) = await _attendanceRepo.GetDailyAttendanceAsync(targetDate, pageIndex, pageSize);
        ...
        [Authorize(Roles = Roles.Admin)]
        public async Task<IActionResult> GetAllWeeklyHours(DateTime? startDate = null, int pageIndex = 0, int pageSize = 10)
        {
            if (startDate == null)
            {
                var cairoNow = TimeHelper.GetCairoNow();
            ...
            var (data, total) = await _attendanceRepo.GetAllWeeklyHoursAsync(startDateOnly, endDateOnly, pageIndex, pageSize);

...
namespace EmployeeManagementBackend.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<LoginResponseDTO>> SetPassword([FromBody] SetEmployeePasswordRequest request)
        {
            var user = await _userManager.FindByNameAsync(request.PhoneNumber);
            if (user == null)
            {
            ...
            {
                Success = false,
                Message = "Failed to set password: " + string.Join(", ", result.Errors.Select(e => e.Description))
            });
        }
    }
}
